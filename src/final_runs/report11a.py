...
from .common import *
GROUPS = {'supervised': ['logistic_regression','random_forest','xgboost','mlp'], 'temporal': ['transformer_L1','transformer_L64'], 'anomaly': ['autoencoder_B']}
...
def main():
    ...
    for seed in SEEDS:
        for group,names in GROUPS.items():
            for name in names:
                ...
                original_families=pd.read_csv(p/'metrics'/f'{name}_families.csv')
                validation=np.load(p/'metrics'/f'{name}_validation_scores.npy')
                vy,_,_,vids,_=cohort(group,'validation')
                require(lock['cohort_ids_sha256']==__import__('hashlib').sha256(np.asarray(vids,dtype='<i8').tobytes()).hexdigest(),'Threshold cohort hash mismatch')
                require(lock['thresholds']==thresholds(vy,validation,group),'Independent threshold recalibration mismatch')
                for part in ('validation','test'):
                    ...
                    for r in rows:
                        ...
                        for key in ('N','TN','FP','FN','TP','false_positive_rate','false_negative_rate','roc_auc','pr_auc','average_precision'):
                            require(np.isclose(saved[key],r[key],rtol=0,atol=1e-12),'Saved metric mismatch: '+key)
                    for r in fr:
                        saved=original_families[(original_families.partition==part)&(original_families.criterion==r['criterion'])&(original_families.attack_family==r['attack_family'])].iloc[0]
                        require(int(saved.N)==r['N'] and int(saved.detected)==r['detected'],'Family counts mismatch')
                        require(pd.isna(saved.recall) if r['recall'] is None else np.isclose(saved.recall,r['recall'],rtol=0,atol=1e-12),'Family recall mismatch')
                ...
                for t in record['timings']:runtime.append(dict(seed=seed,model=name,group=group,training_seconds=training['training_seconds'],validation_selection_seconds=training.get('validation_selection_seconds',training.get('selection_seconds',training.get('benign_validation_selection_seconds'))),training_stage_seconds=training.get('training_stage_seconds',training.get('stage_seconds')),peak_memory_bytes=max(training['peak_memory_bytes'],record['peak_memory_bytes']),model_size_bytes=size,total_required_model_bytes=pipeline,**t))
        ...
        for part in ('validation','test'):
            ...
            for i,label in enumerate(book):
                # Include benign overlap to document union false alarms.
    ...
    dest=OUT/'aggregate_phase11a';dest.mkdir(exist_ok=True)
    df.to_csv(dest/'per_seed_metrics.csv',index=False);ff.to_csv(dest/'per_seed_family_metrics.csv',index=False)
    ff[ff.attack_family.isin(['Bot','Infilteration'])].to_csv(dest/'bot_infilteration_diagnostics.csv',index=False)
    ...
    deltas=[]
    for treated,control,cn in [('transformer_L64','transformer_L1','phase5_matched')]:
    ...
    write(OUT/'audit/phase11a/final_verification.json',dict(status='PASS',completed_required_fits=35,completed_self_only_fits=0,independent_confusion_and_metric_recomputation=True,cohort_equality=True,completed_seeds=SEEDS))
    (OUT/'PHASE_11A_EXECUTION_SUMMARY.md').write_text('# Phase 11A final execution complete\n\nAll 35 Phase 11A mandatory fits completed freshly across seeds 42, 123, 456, 789, 1024. No development fit was substituted.\n\n'
        'Saved scores, cohort IDs, threshold locks and metrics passed independent verification. Per-seed metrics, five-seed arithmetic means and sample SD, paired differences, family recalls, RF/AE/OR overlaps and runtime measurements are in aggregate_phase11a/. The interrupted earlier seed-42 Transformer-L64 attempt is preserved in audit/phase11a/interrupted_seed_42_temporal/. Graph models were not executed; Phase 11B was not started.\n\n'
